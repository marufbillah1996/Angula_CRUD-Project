...
namespace Core_Server_Side.HostedService
{
    public class DbSeederHostedService : IHostedService
    {
        ...
        public async Task SeedDbAsync(BookSellerDbContext db)
        {
            ...
            if (!db.Authors.Any())
            {
                ...
                var B1 = new Book { BookName = "Book 01", Price = 200.00M, PublishDate = new DateTime(1998, 10, 10), Available = true, Picture = "1.jpg" };
                G1.Books.Add(B1);
                var B2 = new Book { BookName = "Book 02", Price = 250.00M, PublishDate = new DateTime(2000, 10, 10), Available = true,  Picture = "2.jpg" };
                G1.Books.Add(B2);
                var C1 = new Customer { CustomerName = "Customer 01", CustomerPhone = "01684939987" };
                await db.Customers.AddAsync(C1);
                var C2 = new Customer { CustomerName = "Customer 02", CustomerPhone = "01712071254" };
